...
void CPropertyListCtrl::DrawItem(int iItem, DWORD itemState, CDC *pDC, CRect itemRect, BOOL /*bDrawAlways*/)
{
	...
	{
		if (m_bDrawRootIcons)
			DrawRootButton(&m_memDC, iconRect, 0, pItemDesc->dwUserData3);
		else
			if (m_bAllowEmptyRootExpand || (pItemDesc->dwFlags & PLC_ITEM_FLAG_ALLOW_ROOT_EXPAND))
				DrawRootButton(&m_memDC, iconRect, pItemDesc->dwFlags & PLC_ITEM_FLAG_EXPANDED, 0);
	}
	else
	{
	...
	if (pItemDesc->dwFlags & PLC_ITEM_FLAG_ROOT)
	{
		if (m_bDrawRootButtons || m_bDrawRootIcons || m_bAllowEmptyRootExpand || (pItemDesc->dwFlags & PLC_ITEM_FLAG_ALLOW_ROOT_EXPAND))
			itemRect.left  += 16;
	}
	else
	{
...
/////////////////////////////////////////////////////////////////////////////
void CPropertyListCtrl::DrawRootButton(CDC *pDC, CRect rect, BOOL bExpanded, DWORD dwIcon)
{
	if (m_bDrawRootIcons)
	{
		ImageList_DrawEx(m_imageList.GetSafeHandle(), dwIcon, m_memDC.m_hDC, rect.left - 2, rect.top + ((rect.Height() - 16)>>1), 16, 16, CLR_DEFAULT, CLR_DEFAULT, ILD_TRANSPARENT);
	}
	else
	...
	if (iItem == -1)
	{
		...
		else
		{
			if (m_bAllowItemSelection)
			{
				...
				if (m_bEnableMultiSelection && GetItemState(iItem, LVIS_SELECTED|LVIS_FOCUSED))
				{
					BOOL bSelected = pItemDesc->dwFlags & PLC_ITEM_FLAG_SELECTED;

					for (int iItem=0; iItem<GetItemCount(); iItem++)
					{
						if (GetItemState(iItem, LVIS_SELECTED|LVIS_FOCUSED))
						{
							PLC_ITEM_DESC* pItemDesc = GetItemDesc(iItem);
...
/////////////////////////////////////////////////////////////////////////////
void CPropertyListCtrl::DrawRootIcons(BOOL bDrawRootIcons)
{
	m_bDrawRootIcons = bDrawRootIcons;
}
/////////////////////////////////////////////////////////////////////////////
...
{
	GetParent()->PostMessage(UM_EDIT, iItem, (LPARAM)this);
}
/////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////
void CPropertyListCtrl::AddIcon(int nIconID)
{
